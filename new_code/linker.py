...
class Linker:
    ...
    def pointwise_prompt(self, mention, lctx, rctx, cand, instruction_dict):
        system_content = "You're an entity disambiguator. I'll give you the description of entity disambiguation and some tips on entity disambiguation, and you need to pay attention to these textual features: \n\n" 
    ...
    # 输出最终确定的链接实体
    def listwise_prompt(self, mention, lctx, rctx, cands, instruction_dict, prompt_id=0):
        ...
        for i, cand in enumerate(cands):
            candidate_entity = "{}.{}".format(cand['entity_name'], cand['entity_desc'])
            content += "Entity {}:{}\n".format(i + 1, candidate_entity)
        content += "You need to determine which candidate entity is more likely to be the mention. Please refer to the above example, give your reasons, and finally answer serial number of the entity and the name of the entity (like <entity number>:<entity name> ). If all candidate entities are not appropriate, you can answer '-1.None'."
        return system_content, content

    # 输入pointwise-pr，输出结果
    def point_wise_el(self, system_content, user_content, cand):
        msgs = [
